...
namespace Buzzware.Cascade.Test {
	...
	public class MaintainsAssociationsTests {
		...
		public async Task MaintainHasManyAssociation() {
			...
			// Test Create
			var initialChildren = parent.Children;	// pinch hildren from parent
			...
			// Test Update
			var updatedParent = await cascade.Update(parent, new Dictionary<string, object?> {[nameof(Parent.colour)] = "yellow" });
			Assert.That(updatedParent!.Children, Is.SameAs(initialChildren));
		...
		public async Task MaintainHasOneAssociation() {
			...
			Assert.That(initialDetail,Is.Not.Null);
			...
			// Test Update
			var updatedChild = await cascade.Update(child, new Dictionary<string, object?> {[nameof(Child.tally)] = 6 });
			Assert.That(updatedChild!.Detail, Is.SameAs(initialDetail));
